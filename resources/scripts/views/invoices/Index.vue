<template>
  <BasePage>
    ...
    >
      ...
      <BaseInputGroup :label="$t('general.sort_by')">
        <BaseMultiselect
          v-model="filters.sort_by"
          :groups="true"
          :options="sort_by"
          :placeholder="$t('general.select_a_sort_by')"
      ...
      <router-link
        ...
        :to="`/filtered-invoices-list?selectedCompany=${companyStore.selectedCompany.id}&customer_id=${filters.customer_id}&status=${filters.status}&from_date=${filters.from_date}&to_date=${filters.to_date}&invoice_number=${filters.invoice_number}&sort_by=${filters.sort_by}`"
        target="_blank"
      >
    ...
    <div v-show="!showEmptyScreen" class="relative table-container">
      ...
      >
        ...
        <template #cell-number="{ row }">
          <router-link
            :to="{ path: `credit-note/${row.data.number_id}/view` }"
            class="font-medium text-primary-500"
          >
            {{ row.data.number }}
          </router-link>
...
function setSortBy(val) {
  switch (val) {
    case 'DATE':
      ...
      break
    case 'INVOICE_NUMBER':
      filters.sort_by = 'INVOICE_NUMBER'
      break
    default:
      filters.sort_by = 'DATE'
      break
